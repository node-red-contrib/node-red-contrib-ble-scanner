...
export function matchesBleDevice(
  ...
  if (namePrefix && name.startsWith(namePrefix)) return true;

  if (!serviceUuid) return !namePrefix;
...
export function uniqueDevices(devices: BleDiscoveredDevice[]): BleDiscoveredDevice[] {
  const byKey = new Map<string, BleDiscoveredDevice>();
...
export function displayDevice(device: BleDiscoveredDevice): string {
